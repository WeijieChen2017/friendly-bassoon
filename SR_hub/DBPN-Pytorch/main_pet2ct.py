...
parser.add_argument('--hr_train_dataset', type=str, default='score_1')
...
parser.add_argument('--prefix', default='NAC2CT', help='Location to save checkpoint models')
...
def train(epoch):
    epoch_loss = 0
    model.train()
    nii_list = glob.glob("./dataset/"+opt.hr_train_dataset+"/*_CTAC.nii.gz")
    nii_list.sort()
    ...
    for idx_t in range(n_dataset):
        ...
        input_nii = nib.load(nii_path.replace("CTAC", "NACS")).get_fdata()
        target_nii = nib.load(nii_path).get_fdata()
        bicubic_nii = nib.load(nii_path.replace("CTAC", "NACB")).get_fdata()
